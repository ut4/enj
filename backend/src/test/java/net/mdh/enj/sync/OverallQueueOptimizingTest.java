...
public class OverallQueueOptimizingTest extends QueueOptimizingTestCase {
    ...
    public void optimisaatiotToimiiYhdessäComplex2() throws IOException {
        ...
        "]");
        List<SyncQueueItem> expected = new ArrayList<>();
        expected.add(SyncQueueUtils.clone(input.get(0), input.get(18).getData()));
        expected.add(SyncQueueUtils.clone(input.get(1), this.makeBatch(
            ((List)input.get(1).getData()).get(0),
            input.get(10).getData(),
            ((List)input.get(5).getData()).get(1)
            // Viimeinen poistetaan kohdassa 17
        )));
        // Pitäisi poistaa (5)
        expected.add(SyncQueueUtils.clone(input.get(2), this.makeBatch(
            input.get(2).getData(),
    ...
    public void optimisaatiotToimiiYhdessäComplex3() throws IOException {
        ...
        expected.add(SyncQueueUtils.clone(input.get(1), input.get(4).getData()));
        expected.add(SyncQueueUtils.clone(input.get(0), this.makeBatch(
            input.get(13).getData(),
