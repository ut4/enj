...
QUnit.module('stat/StatOverviewView', hooks => {
    ...
    function getExpectedDuration(stats: Enj.API.Statistics, nth): string {
        switch (nth) {
            case 0 :
                return `${getExpectedReadableDuration(stats.totalWorkoutTime)}(Yhteensä)`;
            case 1 :
                return `${getExpectedReadableDuration(stats.averageWorkoutTime)}(Keskimäärin)`;
            case 2 :
                return `${getExpectedReadableDuration(stats.longestWorkoutTime)}(Pisin)`;
            case 3 :
                return `${getExpectedReadableDuration(stats.shortestWorkoutTime)}(Lyhin)`;
        }
    }
